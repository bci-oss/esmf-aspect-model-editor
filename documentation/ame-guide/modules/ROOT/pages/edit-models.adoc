...
Every element created in the editor can be altered.
...
The Aspect Model Editor guides you to follow all those BAMM naming conventions for your Aspect Models.
...
2. The Workspace sidebar also offers all available default elements for your Aspect Models.
From here you can add new Properties, Characteristics, Entities, Constraints, Operations, Events, and Traits by dragging an element from the sidebar to the graph area canvas.
...
NOTE: In order to prevent formatting issues, connect a new element right after you added it.
See also xref:known-issues.adoc[known issues].

[[searching-elements]]
== Find elements

Use the search bar in the xref::ui-overview.adoc#toolbar[toolbar] to find elements already connected in the Aspect Model.
By selecting an element, it will be highlighted and centered on the graph area canvas.
...
*Use drag-and-drop from the Workspace sidebar To add abstract entity, proceed as follows:*
